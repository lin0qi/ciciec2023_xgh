...
GPIO LDR R6, =0xFF ;GPIO INPUT ENABLE VALUE
     STR R6, [R4] ;Set input ENABLE
     LDR R5, [R3] ;read GPIO value
	 
     LDR R6, =0xFF ;GPIO OUTPUT ENABLE VALUE
     STR R6, [R4] ;Set OUTPUT ENABLE
	 
     LDR R6, =0XFE ;GPIO_0 Set value
     STR R6, [R2] ;store
     MOVS R1, #1 ;Interval cnt initial
	 BL delay
     LDR R6, =0XFD ;GPIO_1 Set value
     STR R6, [R2] ;store
     MOVS R1, #1 ;Interval cnt initial
     BL delay
     LDR R6, =0XFB ;GPIO_2 Set value
     STR R6, [R2] ;store
     MOVS R1, #1 ;Interval cnt initial
     BL delay
     LDR R6, =0XF7 ;GPIO_3 Set value
     STR R6, [R2] ;store
     MOVS R1, #1 ;Interval cnt initial
     BL delay
     B GPIO
